...
import static org.apache.beam.examples.complete.kafkatomysql.kafka.consumer.Utils.configureKafka;
import static org.apache.beam.examples.complete.kafkatomysql.kafka.consumer.Utils.configureSsl;
import static org.apache.beam.examples.complete.kafkatomysql.kafka.consumer.Utils.getKafkaCredentialsFromVault;
import static org.apache.beam.examples.complete.kafkatomysql.kafka.consumer.Utils.isSslSpecified;
import static org.apache.beam.examples.complete.kafkatomysql.kafka.consumer.Utils.parseKafkaConsumerConfig;
...
import org.apache.beam.examples.complete.kafkatomysql.transforms.FormatTransform;
...
/**
 * The {@link KafkaToMysql} pipeline is a streaming pipeline which ingests data in JSON format from
 ...
 * <li> Modify write step in the {@link KafkaToMysql} by put your Schema class to "writeAvrosToPubSub" step.
 * </ul>
 * </pre>
 */
public class KafkaToMysql {
	...
	/* Logger for class.*/
	private static final Logger LOG = LoggerFactory.getLogger(KafkaToMysql.class);
	...
	public static void main(String[] args) {
		KafkaToMysqlOptions options =
			PipelineOptionsFactory.fromArgs(args).withValidation().as(KafkaToMysqlOptions.class);
	 ...
	 */
	public static PipelineResult run(Pipeline pipeline, KafkaToMysqlOptions options) {
		...
		if (options.getSecretStoreUrl() != null && options.getVaultToken() != null) {
			...
			kafkaConfig = configureKafka(credentials.get(KafkaToMysqlConstants.KAFKA_CREDENTIALS));
		} else {
			LOG.warn(
		...
		JdbcIO.Write<KV<String, String>> write = JdbcIO.<KV<String, String>> write()
			...
			.withPreparedStatementSetter((JdbcIO.PreparedStatementSetter<KV<String, String>>) (element, query) -> {
		...
		pipeline
			...
			//.apply("createValues", Values.create()) //use this to get values only
			//.apply("writeToMysql", new FormatTransform.FormatOutput(options)); // use to convert via avro
			.apply("writeToMysql", write);
